...
**🌐 线上访问**:**<https://jingju.llxww.com/>**(HTTPS · Let's Encrypt 证书,HTTP 自动 301 跳转;超管账号 `admin`,初始密码由部署方设置,登录后请及时修改)

**交付状态**:M1–M4 全部完成。发布通道**走浏览器自动化**(`BrowserChannel` / 教学 `MockChannel`)而非微信 API——未认证个人订阅号无 API 发布权;含**「扫码授权 + 2 天有效期」窗口内无人值守全自动发文**机制(登录态过期号任务原地挂起不进死信、续扫后自动恢复)。18 张表、统一发文状态机、`wx-gateway` 单出口,**129 后端测试通过**。线上以 Docker Compose 部署(mysql/redis/api/nginx/worker+Chromium/beat/certbot)。下方 §一 起按 M1 地基逐层展开,M2–M4 增量见 §九。
